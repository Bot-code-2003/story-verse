// /api/authors/[username]/saved/route.js
...
export async function GET(req, { params }) {
  try {
    ...
    const totalCount = await StorySave.countDocuments({ user: userDoc._id });

    // Fetch saved story IDs with pagination
    const savedDocs = await StorySave.find({ user: userDoc._id })
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean();

    const storyIds = savedDocs.map((doc) => doc.story);
    ...
    console.error("GET /api/authors/[username]/saved error:", err);
